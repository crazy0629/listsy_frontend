...
import { ToysFilter } from "./filters/toy";
import { toyFilter } from "./fiterData";
...
export const ToysSection: React.FC<ToysProps> = ({ page }) => {
  ...
  const onTabClick = (_, value) => {
    const selectedTab = toyFilter[value];
  ...
  return (
    <Styled.MainPageSectionWrapper>
      <Styled.FilterTabWrapper>
        <Tabs
          activeTab={toyFilter.indexOf(
            toyFilter.filter((f) => f.page === page)[0]
          )}
