...
import { petFilter } from "./fiterData";
import { Tabs, Tab } from "react-tabs-scrollable";
import { IoIosArrowBack, IoIosArrowForward } from "react-icons/io";
import { SERVER_URI } from "@/config";
import axios from "axios";
import { toast } from "react-toastify";
import { PetFilter } from "./filters/pet";

type ServiceProps = {
  page?: string;
};

export const PetSection: React.FC<ServiceProps> = ({ page }) => {
  ...
  const onTabClick = (_, value) => {
    const selectedTab = petFilter[value];
  ...
  return (
    <Styled.MainPageSectionWrapper>
      <Styled.FilterTabWrapper>
        <Tabs
          activeTab={petFilter.indexOf(
            petFilter.filter((f) => f.page === page)[0]
          )}
      ...
      <Styled.MainGridWrapper
        className={isShowFilter && page !== "/pets/all-pets" ? "filtered" : ""}
      >
        {loading ? (
          ...
          <InfiniteScroll
            ...
            className={
              isShowFilter && page !== "/pets/all-pets" ? "filtered" : ""
            }
          ...
          <Styled.FilterSection className={isShowFilter ? "active" : ""}>
            ...
            <div className="filter-wrapper">
              <PetFilter
                ...
                itemCategory={petFilter.filter((f) => f.page === page)[0].label}
                page={page}
              />
            </div>
